...
import com.puskesmas.inventory.web.model.ObatMasuk;
...
/**
 * Data Access Object for ObatMasuk (Incoming Medicine) entity.
 */
public class ObatMasukDao extends BaseDao {

    /**
     * Get all incoming medicine records.
     */
    public List<ObatMasuk> findAll() throws SQLException {
        String sql = "SELECT om.*, o.nama_obat, o.satuan FROM obat_masuk om " +
    ...
    /**
     * Create a new incoming medicine record.
     */
    public void create(ObatMasuk data) throws SQLException {
        String sql = "INSERT INTO obat_masuk (id_obat, tanggal_masuk, jumlah, nomor_batch, " +
                "supplier, keterangan, created_by) VALUES (?,?,?,?,?,?,?)";
        ...
        try (Connection conn = getConnection();
            ...
            ps.executeUpdate();
            log.info("Incoming medicine recorded: {} qty {}", data.getNamaObat(), data.getJumlah());
        }
    }

    private ObatMasuk mapResultSet(ResultSet rs) throws SQLException {
        ...
        if (tglMasuk != null) {
            m.setTanggalMasuk(tglMasuk.toLocalDate());
        }
